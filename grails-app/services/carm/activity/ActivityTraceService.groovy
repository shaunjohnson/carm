...
class ActivityTraceService {
    ...
    /**
     * Counts the total number of activity events generated by a user.
     *
     * @param user User used for query
     * @return Number of ActivityTrace objects
     */
    int countActivityByUser(User user) {
        ActivityTrace.countByUsername(user.username)
    }
     ...
     * @return List of ActivityTrace objects
     */
    List<ActivityTrace> listActivityByUser(User user, Map params) {
        ActivityTrace.findAllByUsername(user.username, buildQueryParams(params))
    }
